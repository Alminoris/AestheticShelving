...
public class LadderShelfScreen extends HandledScreen<LadderShelfScreenHandler>
{
    ...
    protected void drawForeground(MatrixStack matrices, int mouseX, int mouseY)
    {
        textRenderer.draw(matrices, this.title.asOrderedText(), this.titleX,
                this.titleY, WOOD_COLORS.get(getWoodName(ModBlocks.LADDER_SHELVES, handler.blockEntity)));
        textRenderer.draw(matrices, this.playerInventoryTitle.asOrderedText(),
                this.playerInventoryTitleX, this.playerInventoryTitleY, WOOD_COLORS.get(getWoodName(ModBlocks.LADDER_SHELVES, handler.blockEntity)));
    }
    ...
    protected void drawBackground(MatrixStack context, float delta, int mouseX, int mouseY)
    {
        ...
        drawTexture(context, x, y, 0, 0, backgroundWidth, backgroundHeight+16);
    }
